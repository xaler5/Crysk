...
//-------------------Internal functions-------------------------------------------------//
    ...
    function calculateAttackNumbers(address adr, string to, uint quantity) internal view returns (bool) {
        ...
        uint defense = countries.getArmy(to).mul(players.getLevel(attackedPlayer).mul(dmg));
