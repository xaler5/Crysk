...
App = {
  ...
  setNickName: function (name) {
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.Crysk.deployed().then(function(instance) {
        ...
        return cryskInstance.setNickName(name);
  ...
  listenToEvents: function() {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.Crysk.deployed().then(function(instance) {
        cryskInstance = instance;
        return cryskInstance;
      }).then(function(result) {
          newPlayerEvent = cryskInstance.playerCreated();
          buyEvent = cryskInstance.buyEvent();
          bombEvent = cryskInstance.dropBomb();
          nextLevelEvent = cryskInstance.nextLevel();
          attackInflictedEvent = cryskInstance.attackInflicted();
          newOwnerEvent = cryskInstance.newOwner();
          changedTeamEvent = cryskInstance.changedTeam();
          buyBombEvent = cryskInstance.buyBombs();
          buyArmyEvent = cryskInstance.buyArmies();
          armyAddedEvent = cryskInstance.armyAdded();
      }).then(function() {
        ...
        nextLevelEvent.watch(function(error, result){
          if (!error) {
            ...
            if (document.getElementById('logOpen').style.display == "block") blink('#logOpen');
            //SHOW NEXT LEVEL DIV CONGRATS !! CHOOSE an Upgrade between upgrading Army damage or bomb damage +33%
